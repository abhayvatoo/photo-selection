...
const stripeSecretKey =
  ...
  (isDevelopment ? 'sk_test_development_dummy_key' : '');
...
export const STRIPE_CONFIG = {
  ...
  publishableKey:
    ...
    (isDevelopment ? 'pk_test_development_dummy_key' : ''),

  // Webhook secret for verifying Stripe events
  webhookSecret:
    process.env.STRIPE_WEBHOOK_SECRET ||
    (isDevelopment ? 'whsec_development_dummy_secret' : ''),
  ...
  products: {
    ...
    professional: {
      priceId:
        process.env.STRIPE_PROFESSIONAL_PRICE_ID || 'price_professional_dev',
      amount: 2900, // $29.00
    },
    enterprise: {
...
export const createDevelopmentSubscription = async (
  ...
  if (!STRIPE_CONFIG.isDevelopment) {
    throw new Error(
      'Development subscription creation only available in development mode'
    );
  }
...
export const getPlanTypeFromPriceId = (
  priceId: string
