...
export async function checkPhotoLimit(
  ...
  return {
    ...
    allowed:
      ...
      currentPhotos < limits.maxPhotosPerWorkspace,
...
export async function checkUserLimit(
  ...
  return {
    ...
    allowed:
      ...
      currentUsers < limits.maxUsersPerWorkspace,
...
export async function createDevelopmentSubscription(
  ...
  if (!STRIPE_CONFIG.isDevelopment) {
    throw new Error(
      'Development subscriptions only available in development mode'
    );
  }
  ...
  const subscription = await prisma.subscription.upsert({
    ...
    update: {
      ...
      stripePriceId:
        STRIPE_CONFIG.products[
          planType.toLowerCase() as keyof typeof STRIPE_CONFIG.products
        ].priceId,
    },
    create: {
      userId,
