...
function validatePaginationParams(searchParams: URLSearchParams) {
  ...
  const limit = Math.min(
    MAX_PAGE_SIZE, 
    Math.max(MIN_PAGE_SIZE, parseInt(searchParams.get('limit') || DEFAULT_PAGE_SIZE.toString()) || DEFAULT_PAGE_SIZE)
  );
...
async function getUserWorkspaces(userId: string, userRole: UserRole): Promise<{ id: string }[]> {
...
export async function GET(request: NextRequest) {
  return withErrorHandler(async () => {
    ...
    const accessibleWorkspaces = await getUserWorkspaces(session.user.id, userRole);
