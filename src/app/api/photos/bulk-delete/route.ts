...
const bulkDeleteSchema = z.object({
  photoIds: z.array(z.number().int().positive())
    ...
    .max(MAX_BULK_DELETE_SIZE, `Cannot delete more than ${MAX_BULK_DELETE_SIZE} photos at once`)
});

// Validate user permissions for bulk delete
async function validateBulkDeletePermissions(userId: string, userRole: UserRole): Promise<boolean> {
...
async function getDeleteablePhotos(photoIds: number[], userId: string, userRole: UserRole) {
...
export async function DELETE(request: NextRequest) {
  return withErrorHandler(async () => {
    ...
    return withCSRFProtection(request, async () => {
      ...
      const deletablePhotos = await getDeleteablePhotos(photoIds, userId, userRole);

      if (deletablePhotos.length === 0) {
        return NextResponse.json({ 
          error: 'No photos found or insufficient permissions to delete selected photos' 
        }, { status: 404 });
      }
