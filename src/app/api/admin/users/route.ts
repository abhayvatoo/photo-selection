...
const createUserSchema = z.object({
  ...
  role: z.enum(['SUPER_ADMIN', 'BUSINESS_OWNER', 'STAFF', 'USER']),
  workspaceId: z
    .string()
...
const getUsersSchema = z.object({
  workspaceId: z
    ...
    .regex(/^c[a-z0-9]{24}$/, 'Invalid workspace ID format')
    .optional(),
  role: z.enum(['SUPER_ADMIN', 'BUSINESS_OWNER', 'STAFF', 'USER']).optional(),
});
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (
      !csrfToken ||
      !(await CSRFProtection.validateToken(session.user.id, csrfToken))
    ) {
      return NextResponse.json(
