...
```typescript
workspaceId: z.string().regex(/^c[a-z0-9]{24}$/, 'Invalid workspace ID format - must be a valid CUID')
```
...
const schema = z.object({
  workspaceId: z.string().regex(/^c[a-z0-9]{24}$/, 'Invalid workspace ID format')
});
...
if (limits.maxPhotosPerWorkspace !== -1 && currentCount >= limits.maxPhotosPerWorkspace) {
